...
contract Staking is IStaking, OwnableUpgradeable {
    ...
    function _processAllCheckpoints(address staker) internal {
        uint256[] memory rewardsBreakdown = getRewardsDetails(staker);
    ...
    function _claimRewards() internal returns (uint256 unclaimedRewards) {
        address sender = msg.sender;
    ...
    function claimRewards() external {
        uint256 unclaimedRewards = _claimRewards();
        SafeERC20.safeTransfer(IERC20(usdToken), msg.sender, unclaimedRewards);
    }

    function distributeRewards(uint256 amount) external {
        require(msg.sender == rewardDistributor, 'Unauthorized to distribute rewards');
        require(amount > 0, "must distribute rewards greater than zero");
        _processAllSections();
        uint64 currentTime = uint64(block.timestamp);
        address sender = msg.sender;
        SafeERC20.safeTransferFrom(
    ...
    function getRewardsClaimable(
        address staker
    ) public view returns (uint256 rewardsClaimable) {
        uint256[] memory rewardsBreakdown = getRewardsDetails(staker);
        for (uint64 i = toClaimCheckpoint[staker]; i < numCheckpoints; i++) {
            rewardsClaimable += rewardsBreakdown[i];
        }
    }
    ...
    function getRewardsDetails(
        ...
        for (uint64 i = 0; i < numCheckpoints; i++) {
            if (i < checkpointIndexes[staker]) {
                ...
                rewardsBreakdown[i] =
                    (stakerPoint * checkpoint.rewards) /
                    totalPoint;
            }
        }
        ...
        returns (GlobalRewardsDetails[] memory)
    {
        ...
        for (uint64 i = 0; i < numCheckpoints; i++) {
            ...
            globalRewardsDetails[i].rewardsAmount = checkpoints[i].rewards;
        }
